...
class FanMonitorWorker(QRunnable):
    ...
    def run(self):
        ...
        try:
            # KHỞI TẠO WMI TRONG WORKER THREAD
        ...
        while self.is_running:
            try:
                ...
                # CHỈ LẤY STATUS CODE
                current_status = self.fan_monitor.check_fan_status(fan_rpm)
                ...
                if self.local_previous_status != current_status:
                    ...
                    # XỬ LÝ TRƯỜNG HỢP fan_rpm LÀ None
                    safe_fan_rpm = fan_rpm if fan_rpm is not None else 0.0
                    self.signals.status_changed.emit(current_status, safe_fan_rpm)
...
class StartupWindow(QWidget):
    def __init__(self, font_family=None, parent=None):
        ...
        # self.fan_monitor = FanMonitor(simulation_mode=True)
        ...
        self.server_url = "https://rosaai_server1.rosachatbot.com/health"
    ...
    def check_connection(self):
        """Kiểm tra kết nối tới server nội bộ"""
        try:
            url = QUrl("https://rosaai_server1.rosachatbot.com/health")
            request = QNetworkRequest(url)
            request.setTransferTimeout(3000)  # 3 giây timeout
            request.setHeader(QNetworkRequest.ContentTypeHeader, "application/json")

            # Gửi POST rỗng để kiểm tra server health
    ...
    def handle_network_reply(self, reply):
        ...
        try:
            if reply.error() == QNetworkReply.NoError:
                ...
                # Thử parse JSON phản hồi
                import json
                try:
                    data = json.loads(response_data)
                    if data.get("success") is True:
                        self.is_online = True
                        print(f"✅ Kết nối server thành công: {data.get('text', '')}")
                    else:
                        self.is_online = False
                        print(f"⚠️ Server phản hồi lỗi: {data}")
                except json.JSONDecodeError:
                    self.is_online = False
                    print("⚠️ Không đọc được phản hồi JSON từ server")
            else:
                self.is_online = False
                print(f"❌ Lỗi kết nối server: {reply.errorString()}")
    ...
    def create_monitoring_section(self):
        ...
        text_label = QLabel("Thiết bị đang được kiểm tra")
    ...
    def create_divider(self):
        """Tạo một đường kẻ ngang giữa các hàng"""
        divider = QFrame()
    ...
    def create_current_info_section(self):
        ...
        # --- Container chỉ cho 4 thông tin đăng ký ---
        ...
        if current_info:
            ...
            # HÀNG 1
            row = self.create_info_display_row("Tên công ty:", reg_info.get("company_name", ""))
            container_layout.addWidget(row)
            container_layout.addWidget(self.create_divider())

            # HÀNG 2
            row = self.create_info_display_row("Tên máy:", reg_info.get("machine_name", ""))
            container_layout.addWidget(row)
            container_layout.addWidget(self.create_divider())

            # HÀNG 3
            row = self.create_info_display_row("Email:", reg_info.get("email", ""))
            container_layout.addWidget(row)
            container_layout.addWidget(self.create_divider())

            # HÀNG 4 — ngày đăng ký (chỉ hiển thị)
            reg_date = reg_info.get("registration_date", "")
            if reg_date:
                try:
                    date_obj = datetime.fromisoformat(reg_date)
                    formatted_date = date_obj.strftime("%d/%m/%Y %H:%M")
                    date_row = self.create_info_row("Đăng kí:", formatted_date)
                    container_layout.addWidget(date_row)
                except:
                    pass
        else:
        ...
        # Thêm container thông tin vào layout chính
        main_layout.addWidget(info_container)

        # --- Ghi chú bên dưới (nằm ngoài container) ---
        main_layout.addSpacing(15)
        desc_label = QLabel("Khi phát hiện sự cố hệ thống, email sẽ \nđược gửi tự động đến địa chỉ trên.")
        ...
        # --- Nút chỉnh sửa (nằm ngoài container) ---
        ...
        # THÊM ICON VÀO NÚT
        icon_path = resource_path(r"assets\image\Chỉnh sửa.png")
        if os.path.exists(icon_path):
            icon = QIcon(icon_path)
            self.edit_button.setIcon(icon)
            self.edit_button.setIconSize(QSize(20, 20))  # Điều chỉnh kích thước icon

        main_layout.addWidget(self.edit_button)

        main_layout.addStretch()

        return main_widget  # Trả về widget thay vì wrapper
