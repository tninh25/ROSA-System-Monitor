...
from backend.fan_monitor import FanMonitor
...
class FanMonitorSignals(QObject):
    status_changed = pyqtSignal(str, float)  # status, rpm
...
class FanMonitorWorker(QRunnable):
    def __init__(self, fan_monitor):
        ...
        self.local_previous_status = "000"  # THÊM: Lưu trạng thái cục bộ
    ...
    def run(self):
        ...
        while self.is_running:
            try:
                ...
                # Kiểm tra thay đổi trạng thái SO VỚI TRẠNG THÁI CỤC BỘ
                if self.local_previous_status != current_status:
                    ...
                    # Cập nhật trạng thái cục bộ SAU KHI phát signal
                    self.local_previous_status = current_status
                else:
                    print(f"🔍 WORKER: Status unchanged: {current_status}")
                
                # Sleep trong thread worker - không ảnh hưởng UI
                import time
...
class StartupWindow(QWidget):
    ...
    def create_info_display_row(self, label, value):
        ...
        row_layout.addWidget(line_edit)  # ⬅️ Thêm cả line_edit nhưng ẩn
    ...
    def on_update_clicked(self):
        ...
        if updates:
            ...
            if success:
                ...
                self.exit_edit_mode()  # ⬅️ Thoát chế độ chỉnh sửa sau khi cập nhật thành công
            else:
                self.popup_manager.show_error(message)
        else:
            self.popup_manager.show_info("Không có thông tin nào được thay đổi")
            self.exit_edit_mode()  # ⬅️ Thoát chế độ chỉnh sửa nếu không có thay đổi
    ...
    def on_fan_status_changed(self, new_status, fan_rpm):
        """Xử lý khi trạng thái quạt thay đổi - chạy trên main thread"""
        try:
            # LẤY OLD_STATUS TỪ FANMONITOR (trước khi thay đổi)
            old_status = self.fan_monitor.current_status
            
            # Cập nhật trạng thái hiện tại của FanMonitor
            ...
            # Hiển thị popup từ file frontend/popup
            self.show_fan_popup(new_status)
            
            # LUÔN GỬI REQUEST KHI CÓ THAY ĐỔI TRẠNG THÁI
            if new_status != old_status:
                print(f"🔍 UI: Status changed, preparing to send request...")
                ...
                server_data = {
                    ...
                    'old_status_text': "Normal" if old_status == "000" else "Fan Error",
                    'new_status_text': "Normal" if new_status == "000" else "Fan Error",
                    'fan_speed': fan_rpm,
                    'selected_fan': self.fan_monitor.config.get('selected_fan', 'CPU Fan #1')
                }
                
                # GỬI REQUEST
                print(f"🔍 UI: Calling send_status_to_server")
                self.fan_monitor.send_status_to_server(server_data)
            else:
                print(f"🔍 UI: No status change, skipping request")
            
            # In thông báo console để debug
            status_messages = {
                "000": "Normal", 
                "001": "Fan Error"
            }
            print(f"🎯 STATUS CHANGED: {status_messages[old_status]} → {status_messages[new_status]} | RPM: {fan_rpm}")
    ...
    def show_fan_popup(self, status):
        """Hiển thị popup thông báo trạng thái quạt"""
        try:
            if status == "000":
                popup = PopupMessage("normal", font_family=self.font_family)
            elif status == "001":
                popup = PopupMessage("fan", font_family=self.font_family)
