...
class FanMonitor:
    def __init__(self, config_file: str = "config/settings.json", state_file: str = "last_state.json", simulation_mode=False):
        ...
        self.simulation_scenarios = {
            "normal": 1500,      # RPM bình thường
            "stopped": 0,        # Quạt dừng
            "slow": 300,         # Quạt quay chậm
            "fast": 3000,        # Quạt quay nhanh
            "error": None        # Lỗi sensor
        }
    ...
    def get_simulated_reading(self) -> Optional[float]:
        """Lấy giá trị RPM giả lập cho testing"""
        scenarios = [
            # Bình thường trong 10 giây (5 lần * 2 giây)
            *["normal"] * 5,           # 10 giây bình thường
            
            # # Quạt dừng trong 10 giây (5 lần * 2 giây) → 001
            # *["stopped"] * 5,          # 10 giây dừng
            
            # # Bình thường trong 10 giây (5 lần * 2 giây)
            # *["normal"] * 5,           # 10 giây bình thường
            
            # # Quạt chậm trong 30 giây (15 lần * 2 giây) → 002
            # *["slow"] * 15,            # 30 giây chậm
            
            # # Bình thường trong 10 giây (5 lần * 2 giây)
            # *["normal"] * 5,           # 10 giây bình thường
            
            # Quạt nhanh trong 20 giây (10 lần * 2 giây) → 003
            *["fast"] * 10,            # 20 giây nhanh
            
            # Bình thường trong 10 giây (5 lần * 2 giây)
            *["normal"] * 5,           # 10 giây bình thường
            
            # Lỗi sensor trong 6 giây (3 lần * 2 giây) → 001
            *["error"] * 3,            # 6 giây lỗi
        ...
        if self.simulation_step >= len(scenarios):
            self.simulation_step = 0  # Quay lại từ đầu
        
        current_scenario = scenarios[self.simulation_step]
        self.simulation_step += 1
        
        print(f"[SIMULATION] Step {self.simulation_step}: {current_scenario}")
        return self.simulation_scenarios[current_scenario]
    ...
    def initialize_sensor_min_max(self):
        ...
        if not self.sensor_min_max_initialized:
            if self.simulation_mode:
                # Dữ liệu min/max giả lập cho testing
                self.sensor_min_max['fans'][self.selected_fan_name] = {
                    'min': 500,   # RPM tối thiểu
                    'max': 2000   # RPM tối đa
                }
                self.sensor_min_max_initialized = True
                print(f"✅ [SIMULATION] Initialized fan: {self.selected_fan_name} (Min: 500, Max: 2000)")
                return
